...
用户工作目录初始化：六项配置 + 双宿主骨架落地 + 环境检测。**不是 STAGE_ORDER 的一员**，不写 state.json、无门禁；任何时候可重跑。
...
4. **点收与转述**：核对 `.reproduce.json`、templates/common、`.codex/agents`、pyproject、目录树和环境检测。转述给用户时讲人话：
   - uv 缺失 → 给安装指引，明确这是硬前置；
   - Python 依赖缺失 → 指引在本目录运行 `uv sync`；
   - codex CLI 缺失 → 只影响 Claude Code 主编排时的异构外审，将降级为 Claude 独立盲审并封顶 B；
   - claude CLI 缺失 → 只影响 Codex 主编排时的异构外审，将降级为 Codex 独立盲审并封顶 B；
   - Codex 当前任务若刚安装 `.codex/agents` 尚未发现命名角色，可继续使用通用子代理 + 完整角色合同；新任务自动加载命名角色。
