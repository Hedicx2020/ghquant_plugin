...
7. **mode=code — 对矩阵 done 行逐条核对**：公式符号/窗口/算子/分子分母/时点(t vs t-1)/参数逐一对照实现函数；未登记的简化 = `deviation_undeclared`（core 要素记 **critical**）；实现位置指向 pass/TODO 空壳 = **虚报 critical**，须在 coverage_matrix 变更日志记回退（状态打回 in_progress）；strategy.py 公开函数映射不回要素 ID 的「私货」记 major。
...
- [ ] （code）矩阵 done 行逐条核对；空壳已记 critical 并在变更日志记回退
