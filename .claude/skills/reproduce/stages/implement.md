...
2. **按难度编排**（见 SKILL.md 第六节裁剪矩阵）：
   - **easy**：单 `quant-coder` 一次实现全部 milestone。
   - **medium**：单 `quant-coder` 逐 milestone **串行**。
...
4. **每完成一个 milestone 更新 state**：`uv run python tools/state.py milestone <id> <mid> implement done`（G-IM-4 逐 milestone 核对 implement=done）。hard 的 milestone 级 verify 通过后 `milestone <id> <mid> verify done`；auditor mode=code 通过后 `milestone <id> <mid> code_review done`。
5. **点收输出合同**（逐一 `ls -la`）：
