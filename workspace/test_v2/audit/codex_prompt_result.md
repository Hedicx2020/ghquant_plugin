...
本次审查对象：report_id = `test_v2`，研报类型 = `timing`，难度 = `hard`，迭代轮数 = `3` / 上限 `6`。

# 输入文件

- `output/test_v2/results/comparison.json` —— 本轮 comparison 对比表（指标级 report_value/reproduced_value/rel_dev/pass）
- `workspace/test_v2/../output/test_v2/verify_report.md`（即 `output/test_v2/verify_report.md`）—— 验证报告，含归因文字
- `workspace/test_v2/assumptions.md` —— 假设登记簿（含预期影响方向）
- `workspace/test_v2/spec/ambiguities.md` —— 歧义清单
- `workspace/test_v2/spec/coverage_matrix.md` —— 覆盖矩阵（含 skipped/infeasible 行及理由）
  ...
  "findings": [
    {
      ...
      "location": "文件:行号或表格锚点，如 output/test_v2/verify_report.md#RankIC 或 comparison.json 的 metrics[3]",
      "description": "……",
      "suggestion": "……",
