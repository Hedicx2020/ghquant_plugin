...
本次审查对象：report_id = `test_v2`，市场 = `多资产`，研报类型初判 = `timing`，难度 = `hard`。
...
- `workspace/test_v2/spec/report_text.md` —— 研报逐页纯文本，`===== PAGE n =====` 分页标记
- `workspace/test_v2/spec/tables_extracted.md` —— pdfplumber 抽取的表格附录
- `workspace/test_v2/spec/spec.md` —— 待审查的复现规格书（**阶段一禁止读取，见下**）
- `workspace/test_v2/spec/coverage_matrix.md` —— 覆盖矩阵
...
打开 `workspace/test_v2/spec/spec.md`，对照你在阶段一独立产出的清单，执行以下五项：
...
1. **每条意见必须给出可定位证据**：页码（如 `p9-p10`）或 `文件:行号`（如 `src/test_v2/main.py:88`）；给不出定位的怀疑不得作为一条 finding 提出。
