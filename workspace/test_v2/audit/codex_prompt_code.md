...
本次审查对象：report_id = `test_v2`，研报类型 = `timing`，标签 = `[]`，难度 = `hard`。此次审查发生在 implement 阶段结束后、verify 阶段开始前——你的判定直接决定是否放行去跑回测。

# 输入文件

- `workspace/test_v2/spec/spec.md` —— 复现规格书（要素 ID 体系，F 类含公式、B 类含回测设置）
- `workspace/test_v2/spec/coverage_matrix.md` —— 覆盖矩阵（含每个要素的「实现位置」列，指向具体文件:函数）
- `workspace/test_v2/assumptions.md` —— 假设登记簿（凡代码里的简化/近似处理，若已在此登记，不算未声明偏差）
- `src/test_v2/` —— 全部实现代码（可自行按需 Read/grep 目录下任意文件）
- `workspace/test_v2/audit/common_signatures.md` —— common/ 下可复用函数的签名摘要（如需要更多细节，可自行 grep `common/` 下对应源文件）
...
5. **作弊模式排查（K1/K5/K6）**：
   - **K1 硬编码**：见下方「R 类数值清单」，在 `src/test_v2/` 下 grep 这些数值的字面量，若命中且该数值出现在计算路径上（而非仅注释/文档字符串）→ 记一条 critical 级 finding，并说明命中位置。
...
```
-2.81%、-3.24%、-3.26%、-3.43%、-3.72%、-7.46%、0.31、0.48、0.49、0.59、0.60、0.81、0.95、1.11、1.17、1.22、1.23、1.31、1.33、1.36、1.37、1.38、1.39、1.40、1.42、1.44、1.45、1.47、1.49、1.52、1.55、1.57、1.58、1.70、1.71、1.73%、1.77、1.98、128.88、129.48、129.96、147.29、2.04、2.17%、2.28%、2.30%、2.35、2.48%、2.67%、2.75%、2.79%、2.89%、2.93%、20.78%、3.01%、3.10%、3.42%、3.43%、3.50%、3.71%、3.74%、3.75%、3.89%、3.99%、32.56%、38.71%、4.40%、4.55%、4.78%、43.38%、44.48%、47.82%、49.02%、49.07%、49.51%、5.10%、5.77%、50.13%、51.18%、51.45%、51.58%、51.65%、52.52%、53.13%、53.98%、54.12%、54.34%、54.42%、54.69%、54.86%、54.93%、55.26%、55.68%、56.31%、56.35%、56.46%、56.58%、56.90%、57.29%、58.55%、59.93%、6.00%、6.40%、62.89%、65.28%、67.99%、7.46%、7.65%、85.22%
```
  ...
  "findings": [
    {
      ...
      "location": "文件:行号，如 src/test_v2/strategy.py:42",
      "description": "……",
      "suggestion": "……",
...
1. **每条意见必须给出可定位证据**：页码或 `文件:行号`（如 `src/test_v2/strategy.py:42`）；给不出定位的怀疑不得作为一条 finding 提出。代码审查的定位应优先给文件:行号，能追溯到具体代码。
